...
def _load_country_mapping() -> dict:
    ...
    path = os.path.join(os.path.dirname(__file__), "assets/slim-2.json")
...
def get_country_code(full_name: str) -> str:
    ...
    for official_name, code in mapping["all"]:
        ...
        if shared:
            # How much of the *query* is covered (so "united kingdom" 2/2 = 1.0
            # against "united kingdom of great britain and northern ireland")
            query_coverage = len(shared) / len(query_words)
            # Penalise if official name is much longer (avoid false positives)
        ...
        # Only when no word overlap and query is long enough
...
def filter_airport_data():
    # This is a utility function to create a smaller airports.csv with only the needed fields
    # and only for European airports. It can be used to speed up lookups in the app.
    working_dir = os.path.dirname(os.path.abspath(__file__))
    input_file = os.path.join(working_dir, "assets/airports.csv")
    output_file = os.path.join(working_dir, "assets/filtered_airports.csv")
    ...
    with open(input_file, 'r', encoding='utf-8') as infile, open(output_file, 'w', encoding='utf-8') as outfile:
        ...
        for line in infile:
            ...
            if parts[2].strip('"').lower() != "large_airport":
                continue
